...
public class ListeTest {
	...
	public void setUp() throws Exception {
		elements = new String[] { "Hello", "Brave", "New", "World" };
	...
	/**
	 * Méthode utilitaire de remplissage de la liste avec les éléments du tableau
	 * #elements
	 */
	...
	public final void testAdd() {
		...
		for (int i = 1; i < elements.length; i++) {
			...
			/*
			 * Attention le précédent "it" a été invalidé par l'ajout Lors du dernier next
			 * le current de l'itérateur est passé à null puisqu'il n'y avait pas (encore)
			 * de suivant, donc retenter un next sur le même itérateur génèrera un
			 * NoSuchelementException. Il faut donc réobtenir un itérateur pour parcourir la
			 * liste après un ajout
			 */
			it = liste.iterator();
			for (int j = 0; j <= i; j++) {
				insertedElt = it.next();
			}
			assertSame(testName + " contrôle ref element[" + i + "] failed", insertedElt, elements[i]);
		}
	}
	...
	public final void testInsert() {
		...
		try {
			...
			assertTrue(testName + " insertion elt null, liste vide failed", liste.empty());
		}
		...
		Iterator<String> it = liste.iterator();
		String insertedElt = it.next();
		assertSame(testName + " contrôle ref element[" + lastIndex + "]  failed", insertedElt, elements[lastIndex]);
		// Si assertSame réussit asserEquals n'est plus nécessaire
		...
		for (int i = 1; i < elements.length; i++) {
			...
			assertSame(testName + " contrôle ref element[" + (lastIndex - i) + "] failed", insertedElt,
	...
	public final void testInsertInt() {
		...
		assertFalse(testName + " insertion elt null ds liste vide failed", result);
		assertTrue(testName + " insertion elt null ds liste vide, liste vide failed", liste.empty());

		// - insertion dans une liste vide avec un index invalide
		result = liste.insert(elements[nextIndex[index]], 1);
		assertFalse(testName + " insertion ds liste vide, index invalide failed", result);
		assertTrue(testName + " insertion ds liste vide, index invalide, " + "liste vide failed", liste.empty());

		// + insertion dans une liste vide avec un index valide
		result = liste.insert(elements[nextIndex[index]], 0);
		// liste = Brave ->
		assertTrue(testName + " insertion ds liste vide, index valide failed", result);
		assertFalse(testName + " insertion ds liste vide, index valide, " + "liste non vide failed", liste.empty());
		index++;

		// - insertion dans une liste non vide avec un index invalide
		result = liste.insert(elements[nextIndex[index]], 5);
		assertFalse(testName + " insertion ds liste non vide, index invalide failed", result);

		// + insertion en début de liste non vide avec un index valide
		result = liste.insert(elements[nextIndex[index]], 0);
		// liste = Hello -> Brave ->
		assertTrue(testName + " insertion début liste non vide, index valide failed", result);
		index++;
		...
		assertTrue(testName + " insertion fin liste non vide, index valide failed", result);
		index++;

		// + insertion en milieu de liste non vide avec un index valide
		result = liste.insert(elements[nextIndex[index]], 2);
		// liste = Hello -> Brave -> New -> World
		assertTrue(testName + " insertion milieu liste non vide, index valide failed", result);
	}
	...
	public final void testSize() {
		...
		assertTrue(testName + " taille liste pleine failed", liste.size() == elements.length);
	}
	...
	public final void testHashCode() {
		...
		assertEquals(testName + " egalite liste hashCode failed", liste.hashCode(), liste2.hashCode(), 0);

		liste2.add("Hourra");
		assertFalse(testName + " inegalite liste equals failed", liste.equals(liste2));
		assertFalse(testName + " inegalite liste hashCode failed", liste.hashCode() == liste2.hashCode());
