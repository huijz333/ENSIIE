...
int mthread_is_empty(mthread_list_t * list) {
    return list->first == NULL;
}

static inline
int mthread_mctx_set(struct mthread_s * mctx, void (*func)(void *), char *stack,
                     size_t size, void *arg) {

    mthread_log("SCHEDULER", "Get context %p\n", mctx);
...
static inline int mthread_mctx_swap(struct mthread_s * cur_mctx,
                                    struct mthread_s * new_mctx) {
...
static void mthread_idle_task(void* arg) {
    ...
    mthread_virtual_processor_t * vp = (mthread_virtual_processor_t*) arg;
    vp->state = 1;
...
int mthread_get_vp_rank() {
    const mthread_virtual_processor_t * vp = mthread_get_vp();
    return vp ? vp->rank : -1;
}

static inline void mthread_init_vp(mthread_virtual_processor_t* vp,
                                   struct mthread_s* idle, struct mthread_s* current, int rank) {
    vp->current = current;
    vp->idle = idle;
...
static inline void mthread_init_lib(long i) {
    ...
    mthread_mctx_set(mctx, mthread_idle_task, stack, MTHREAD_DEFAULT_STACK,
                     &(virtual_processors[i]));
    if (i != 0) {
...
/* Create a thread with given attributes ATTR (or default attributes
 ...
 arguments ARG.  */
int mthread_create(mthread_t * __threadp, const mthread_attr_t * __attr,
